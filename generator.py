...
SYSTEM_BRAND = """Ты — опытный SEO-копирайтер, специализирующийся на брендовых статьях и обзорах.
Пишешь убедительно о конкретных брендах и продуктах, выделяя преимущества и уникальные особенности.
Используешь предоставленные данные о бренде как основу. Стиль — профессиональный, вовлекающий,
ориентированный на целевую аудиторию. Только факты и реальные преимущества."""
...
def _build_prompt(
    ...
    elif article_type == "brand":
        tone_instruction = "ТОН: брендовый, убедительный, акцент на преимуществах и УТП бренда."
    else:
...
def generate_article(
    ...
    system = SYSTEM_EXPERT if article_type == "expert" else (
        SYSTEM_BRAND if article_type == "brand" else SYSTEM_GENERAL
    )
